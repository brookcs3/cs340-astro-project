...
-- Data Definition Queries (DDL) with Reset Stored Procedure
--
-- Northern Oregon Speedrunners Association Database
-- Tracks speedrun submissions, players, games, categories, and platforms.
--
-- The sp_reset_database stored procedure drops all tables, recreates them
-- with the correct schema, and re-inserts all sample data. This allows
-- reviewers to restore the database to its original state after testing CUD ops.

-- Drop the procedure if it already exists so this file is idempotent
...
-- Run the procedure once to initialize the database with schema and sample data
